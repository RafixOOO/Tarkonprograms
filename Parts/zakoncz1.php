  ...
  for($i=0;$i<$length;$i++){
    list($projekt, $detal) = explode(',', $selectedRows[$i]);
        ...
        while ($row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC)) {
            ...
            $sqlinsert = "INSERT INTO dbo.Product_Recznie (Projekt, Pozycja, Ilosc_zrealizowana, Dlugosc_zrealizowana, Maszyna) VALUES ('{$projekt}', '{$detal}', '{$ilosc}', '{$dlugosc}', 'Kooperacyjnie')";
